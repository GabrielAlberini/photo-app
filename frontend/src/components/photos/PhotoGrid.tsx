...
const PhotoGrid: React.FC<PhotoGridProps> = ({
  ...
  const handlePhotoClick = (photo: Photo) => {
    if (selectable && onSelectPhoto) {
      ...
      onSelectPhoto(photo.id, !isSelected);
    } else {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {photos.map((photo) => (
            <div
              ...
              } ${selectedPhotos.includes(photo.id) ? 'ring-4 ring-primary-500' : ''}`}
            ...
            >
              <div className="aspect-w-4 aspect-h-3 w-full overflow-hidden bg-gray-200 dark:bg-gray-700">
                ...
                {/* Selection Overlay */}
                {selectable && (
                  <div className={`absolute inset-0 bg-primary-500 bg-opacity-20 flex items-center justify-center transition-opacity ${
                    selectedPhotos.includes(photo.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-70'
                  }`}>
